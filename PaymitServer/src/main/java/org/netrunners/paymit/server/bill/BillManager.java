...
public class BillManager implements InitializingBean{
	...
	public Bill getBillDetails(String billId){
		Bill bill = this.jdbcTemplate.queryForObject(
		        "select billId, createdTimestamp, owner from t_bill where billId = ?",
		        new Object[]{billId},
		        new RowMapper<Bill>() {
		...
		List<BillItem> rawHistoryItems = jdbcTemplate.query(SQL, new Object[] { billId },
				new BillItemRowMapper());
